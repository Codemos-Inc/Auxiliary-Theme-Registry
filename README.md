...
As being the official repository, the target for this repository is to provide a curated list of themes that are beloved by VS Code users. We want to include **all Marketplace theme extensions down to 50k downloads**. This doesn't mean that we only add the most popular themes in marketplaces, we are open to contributions from the community. We will review the themes and add them to the registry if they meet our quality standards. Our catalog grows rapidly and is updated regularly.
